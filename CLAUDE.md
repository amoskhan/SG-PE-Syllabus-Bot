...
- **Named pairs → student records** (`supabase_lesson_pairs.sql`): the teacher names the Apple/Banana pupil of each pair per lesson (`lesson_pairs`, edited in `PairAssignment.tsx` and the Review Tray). A database trigger (`sync_practice_analysis`) copies each Practice Station AI analysis (`pair_submissions.ai_chat_analysis`) into that student's `skill_analyses` row (`source = 'practice_station'`), re-running when a pupil sends work or the teacher changes a pair. Pupil devices never see names. The Student Dashboard and the Review Tray then share one record and one teacher review (`TeacherReviewPanel`, `teacher_criteria`/`teacher_level` from `supabase_teacher_review.sql`).
