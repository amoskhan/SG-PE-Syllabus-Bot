...
const MarkdownRenderer: React.FC<MarkdownRendererProps> = ({ content }) => {
  ...
  lines.forEach((line, index) => {
    ...
    if (trimmed.startsWith('### ')) {
      ...
      formattedElements.push(<h3 key={index} className="text-lg font-bold text-slate-800 mt-4 mb-2">{formatInline(trimmed.substring(4))}</h3>);
      ...
      formattedElements.push(<h2 key={index} className="text-xl font-bold text-slate-800 mt-5 mb-3">{formatInline(trimmed.substring(3))}</h2>);
    } 
    // List Items
    else if (trimmed.startsWith('- ') || trimmed.startsWith('* ')) {
      inList = true;
      currentList.push(<li key={index} className="text-slate-700">{formatInline(trimmed.substring(2))}</li>);
      ...
      formattedElements.push(<div key={index} className="mb-1 text-slate-700">{formatInline(trimmed)}</div>);
    }
    // Empty lines
    else if (trimmed === '') {
       if (inList) {
    ...
    // Standard Paragraphs
      ...
      formattedElements.push(<p key={index} className="mb-2 text-slate-700 leading-relaxed">{formatInline(trimmed)}</p>);
    }
  });
