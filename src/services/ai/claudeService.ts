    ...
    try {
        ...
        if (poseData && poseData.length > 0) {
            ...
            if (poseData.length > 1) {
                ...
                // `dominantHand` is still used to pick which wrist's Y-track to read for wind-up /
                // backswing checks — drive that off the more-active wrist, not the raw index compare.
